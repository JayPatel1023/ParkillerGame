...
/**
 * Same trapezoid-corner construction as computeTileCorners, but for an open path (a home
 * corridor's ladder, entrance to hub) instead of a closed loop - index 0 and the last index have
 * no wraparound neighbor, so the missing prev/next is synthesized by extending the path's own
 * direction at that end instead of reading off the far end of the array.
 */
export function computeOpenPathTileCorners(worldPoints: [number, number][], i: number, halfWidth: number): [number, number][] {
  const n = worldPoints.length
  const cur = worldPoints[i]

  // A synthesized boundary neighbor extends a fixed, tile-scaled distance from `cur` in the
  // direction away from the real neighbor on the other side - not a full mirror of that
  // neighbor's distance (2*cur - neighbor). Real corridor segments vary a lot in length,
  // especially the last one diving into the hub, and mirroring that length produced a wildly
  // over-stretched, warped tile at the open end (confirmed directly - the last tile rendered as a
  // long distorted sliver reaching well past its own point).
  const extend = (from: [number, number], to: [number, number]): [number, number] => {
    const dx = to[0] - from[0]
    const dy = to[1] - from[1]
    const len = Math.hypot(dx, dy) || 1e-9
    const step = halfWidth * 2
    return [to[0] + (dx / len) * step, to[1] + (dy / len) * step]
  }

  const prev = i > 0 ? worldPoints[i - 1] : extend(worldPoints[1], cur)
  const next = i < n - 1 ? worldPoints[i + 1] : extend(worldPoints[n - 2], cur)
